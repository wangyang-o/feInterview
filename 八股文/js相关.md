...
接受函数作为参数：高阶函数可以将其他函数作为参数传递给它们。这样可以使函数更加灵活和通用，因为它们可以接受不同的行为作为参数。

返回函数：高阶函数可以在其内部创建并返回函数。这使得函数可以动态地生成其他函数，并且返回的函数可以具有对外部作用域的闭包引用。

函数组合：高阶函数可以将多个函数组合在一起形成新的函数。这样的函数通常称为函数组合器，它们接受一个或多个函数，并返回一个新的函数，该函数将按照指定的顺序依次调用传入的函数。

封装行为：高阶函数可以将一些常见的行为或逻辑封装起来，使其可以在不同的上下文中复用。通过将函数作为参数传递，可以将具体的行为从函数中分离出来，使得函数更加通用和可定制。
...
数字会转化为二进制，符号位 1 位+ 指数位 11 位置+ 有效数字 52 位
