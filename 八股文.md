...
- [Mutation可以是异步函数吗](#mutation%E5%8F%AF%E4%BB%A5%E6%98%AF%E5%BC%82%E6%AD%A5%E5%87%BD%E6%95%B0%E5%90%97)
...
浏览器的进程有：浏览器主进程、GPU进程、插件进程、网络进程、渲染进程

# 进程间通信的方式

匿名管道、具名管道、信号量、消息队列、共享内存、socket等

# Mutation可以是异步函数吗

其实试过的都知道就算是异步函数也是可以实现状态的修改，那为什么要都是同步的呢？主要是为了调试，使用devtools的时候采用异步不好追踪数据。一定要注意，mutation不能是异步最主要的就是为了调试，而不是不能修改状态。